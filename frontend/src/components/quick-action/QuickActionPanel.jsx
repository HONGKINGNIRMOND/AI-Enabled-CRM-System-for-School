...
import { Search, Send, User, Phone, BookOpen, TrendingUp, DollarSign, MessageSquare, CheckCircle, AlertCircle, Loader2, ArrowLeft } from 'lucide-react';
...
const QuickActionPanel = () => {
    ...
    const searchStudents = async (term) => {
        ...
        setSearching(true);
        try {
            const response = await api.get(`/quick-action/search-students?search=${term}`);
    ...
    const getStudentData = async (studentId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                generateMessage(response.data.data);
            }
        } catch (error) {
    ...
    const generateMessage = (data) => {
        ...
        if (marks && marks.length > 0) {
            ...
            marks.forEach(mark => {
                msg += `  • ${mark.subject_name}: ${mark.average_marks.toFixed(1)}/100\n`;
            });
        }

        msg += `🏆 Overall Grade Point: ${grade.averageGradePoint.toFixed(2)}\n\n`;

        msg += `💰 *Fee Information*\n`;
        msg += `💳 Pending Amount: ₹${fees.pendingAmount.toFixed(2)}\n\n`;

        msg += `📱 *Parent Contact*\n`;
        msg += `👨 Father: ${parent.father_name || 'N/A'}\n`;
        msg += `👩 Mother: ${parent.mother_name || 'N/A'}\n\n`;
    ...
    const sendWhatsAppMessage = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                setNotification({
                    type: 'success',
                    message: `Message sent successfully to ${response.data.data.parentNumber}`
                });
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <p className="text-gray-600 ml-14">Send student updates to parents via WhatsApp</p>
            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-green-600">
                                        {studentData.parentWhatsApp || 'Not Available'}
                                    </span>
                                </div>
                            </div>
                        )}
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">
                                        {studentData.grade.averageGradePoint.toFixed(2)}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-600 flex items-center gap-1">
                                    ...
                                    <span className="font-medium text-red-600">
                                        ₹{studentData.fees.pendingAmount.toFixed(2)}
                                    </span>
                                </div>
                                        ...
                                        <div className="space-y-1">
                                            {studentData.marks.map((mark, index) => (
                                                <div key={index} className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-medium">{mark.average_marks.toFixed(1)}/100</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
